...
let latestSetState: ((s: BLEState | ((prev: BLEState) => BLEState)) => void) | null = null;
let latestToast: ReturnType<typeof useToast>["toast"] | null = null;
...
async function setupGattConnection(device: BluetoothDevice): Promise<boolean> {
  cleanupListeners();
  ...
  try {
    ...
    console.warn("BLE: Service not found during setup:", e);
    return false;
  }
...
function installDisconnectHandler(device: BluetoothDevice, deviceName: string): void {
  onDisconnectHandler = () => {
    ...
    if (manualDisconnect) {
      ...
      latestSetState?.({ isConnected: false, deviceName: null, isConnecting: false, isReconnecting: false });
      return;
    }

    latestSetState?.({ isConnected: false, deviceName: null, isConnecting: false, isReconnecting: true });

    attemptAutoReconnect(device, deviceName).then((reconnected) => {
      if (!reconnected) {
        if ((window as any)._meshtasticTransport === "ble") {
          (window as any).meshtasticSend = undefined;
          (window as any)._meshtasticTransport = null;
        ...
        latestSetState?.({ isConnected: false, deviceName: null, isConnecting: false, isReconnecting: false });
        latestToast?.({ title: "Disconnected", description: "Node link lost.", variant: "destructive" });
      }
    });
  };
...
async function attemptAutoReconnect(device: BluetoothDevice, deviceName: string): Promise<boolean> {
  if (reconnectInFlight) {
    console.log("BLE: Reconnect already in progress — skipping duplicate");
    return false;
  }
...
export function useBLE() {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      if (!success) {
        setState({ isConnected: false, deviceName: null, isConnecting: false, isReconnecting: false });
        toast({
      ...
      const deviceName = device.name || "Meshtastic Node";
      ...
      toast({ title: "BLE Connected", description: `Bridged to ${deviceName}.` });

      postSystemMessage(`UPLINK ESTABLISHED: Bridged to ${deviceName} via BLE. LoRa terminal active.`);
